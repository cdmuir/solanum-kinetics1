...
conductance'', symbolized as fgmax. This value should be proportional to
the average aperture divided by its maximum aperture for any arbitrary
stomatal density {[}could a figure show how this interacts with stomatal
density?{]}. Leaves operating at fgmax closer to unity will, all else
being equal, will be slower to respond than leaves operating with a
fgmax close to zero, independent of stomatal density. (4) hypothesized
that selection on stomatal density would maintain typical operational
stomatal conductance (gop) relative to gmax at a sufficiently low value
that stomatal conductance would be sensitive to relatively small changes
in guard cell turgor pressure. Consistent will this hypothesis, gop:gmax
is often near 0.25 (5, 6), well below fgmax = 1, and in a range where
\gsw would be responsive to small changes in guard cell turgor pressure.
...
life a leaf, fgmax will change in response to long-term stresses such as
drought. Because guard cell size and aperture typically vary at
...
whereas fgmax will explain more variation within species. Within species
variation can arise from either genetic or environmental differences
between individuals. Neither of these traits along will determine all or
even most of the variance in stomatal kinetics, which is a complex
response to many internal and external signals (cites).
...
guard cell size and fsmax on stomatal kinetics and partition their
...
g\_s = g\_f + (g\_i - g\_f) * exp(-(t/tau)\^{}lambda)

\subsection{Methods}\label{methods}

\subsubsection{Hypothesis generation}\label{hypothesis-generation}

The primary aims and \emph{a priori} hypotheses of this study are
described in (10). The idea to use \gsw response curve to test
hypotheses about which traits influence stomatal kinetics came after the
data had been collected, but not yet analyzed. We started with an
\emph{a priori} hypothesis that guard cell size would influence \(\tau\)
based on a recent metaanalysis of stomatal responses to light (2) and we
planned to compare relationships within and among species, as described
in this paper. We made the decision to test for an association between
...
studies (cite). Therefore, our conclusions about the effect of fgmax on
stomatal kinetics should be interpreted with caution until they are
replicated.

\subsubsection{Stomatal kinetics}\label{stomatal-kinetics}

Equation from (2). Point out that tau matters most.
...
\(\tau\): phylogenetic covariance individual effect mediating treatment
effects (plasticity)

For individual effect of \gcl and \fgmax on \(\tau\) and \(\lambda\): We
used the leave-one-out cross-validation information criterion (LOOIC) to
compare the fit of models using the \emph{R} package \textbf{loo}
version 2.9.0 (\textbf{vehtari\_practical\_2017?}) to calculate LOOIC
values. We selected the model with the lowest LOOIC value
(\textbf{?@tbl-comparison}) to generate posterior predictions for
hypothesis testing.
...
phylogenetic relationships (11, 12). The between- and within-individual
...
component and is estimated following (11); (12) as:
...
average -4.64\% {[}95\% CI: -15.3 to 7.93\%{]} lower than in shade
plants after accounting for other explanatory variables. Within the same
leaf, increasing the measurement light intensity prior to the VPD step
change from \ppfdqty{150} to \ppfdqty{2000} significantly increased
\(\tau\) by 19\% {[}95\% CI: 11.1 to 27.8\%{]}
...
plants, \(\tau\) was on average 12.6\% {[}95\% CI: 10.4 to 14.8\%{]}
greater than in shade plants. Higher measurement light decreased
increased \(\lambda\) slightly by -1.4\% {[}95\% CI: -4.67 to 1.64\%{]}.
...
\hspace{1em}phylogenetic & 18.4 & {}[4.4, 36.6]\\
\hspace{1em}population & 5.6 & {}[0.9, 14.4]\\
\hspace{1em}between-individual & 33.0 & {}[25.2, 39.2]\\
\hspace{1em}within-individual & 42.1 & {}[33.3, 50.1]\\
\addlinespace[0.3em]
\multicolumn{3}{l}{\textbf{$\log(l_\mathrm{gc})$}}\\
\hspace{1em}\hspace{1em}phylogenetic & 56.5 & {}[23.6, 76.0]\\
\hspace{1em}\hspace{1em}population & 5.4 & {}[0.3, 24.2]\\
\hspace{1em}\hspace{1em}between-individual & 37.5 & {}[22.6, 56.3]\\
\addlinespace[0.3em]
\multicolumn{3}{l}{\textbf{$\log(\lambda)$}}\\
\hspace{1em}phylogenetic & 17.8 & {}[6.0, 32.7]\\
\hspace{1em}population & 0.8 & {}[0.0, 5.8]\\
\hspace{1em}between-individual & 69.1 & {}[55.7, 79.7]\\
\hspace{1em}within-individual & 11.1 & {}[4.9, 18.0]\\
\addlinespace[0.3em]
\multicolumn{3}{l}{\textbf{$\log(\tau)$}}\\
\hspace{1em}phylogenetic & 22.6 & {}[8.2, 42.6]\\
\hspace{1em}population & 1.3 & {}[0.0, 7.3]\\
\hspace{1em}between-individual & 53.1 & {}[37.7, 65.8]\\
\hspace{1em}within-individual & 21.5 & {}[15.3, 28.1]\\*
...
(see \textbf{?@tbl-comparison} for model comparison results), there was
a significant phylogenetic correlation between \gcl and \(\tau\)
...
Table~\ref{tbl-fit_amphi}. NOTE THAT BEST MODEL RETAINED INDIVIDUAL
...
\caption{\label{tbl-fit_amphi}Fixed effects (posterior mean, SE, and
...
\hspace{1em}effect of $\log(f_g)$ on $\log(\lambda)$ & 0.12 [0.09, 0.15]\\
\hspace{1em}effect of high measurement light intensity on $\log(\lambda)$ & -0.01 [-0.05, 0.02]\\
\hspace{1em}intercept (shade, low light) & 0.44 [0.32, 0.56]\\
\hspace{1em}effect of sun growth treatment on $\log(\lambda)$ & 0.12 [0.10, 0.14]\\
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{$\log(\tau)$}}\\
\hspace{1em}effect of $\log(f_g)$ on $\log(\tau)$ & 0.21 [0.13, 0.28]\\
\hspace{1em}effect of $\log(l_\mathrm{gc})$ on $\log(\tau)$ & 0.94 [-0.17, 1.94]\\
\hspace{1em}effect of high measurement light intensity on $\log(\tau)$ & 0.17 [0.10, 0.25]\\
\hspace{1em}intercept (shade, low light) & 2.84 [-0.23, 6.01]\\
\hspace{1em}effect of sun growth treatment on $\log(\tau)$ & -0.05 [-0.17, 0.08]\\
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{$\log(f_g)$}}\\
\hspace{1em}effect of high measurement light intensity on $\log(f_g)$ & 0.80 [0.76, 0.84]\\
\hspace{1em}intercept (shade, low light) & -2.63 [-2.94, -2.31]\\
\hspace{1em}effect of sun growth treatment on $\log(f_g)$ & -0.16 [-0.24, -0.08]\\
\addlinespace[0.3em]
\multicolumn{2}{l}{\textbf{$\log(l_\mathrm{gc})$}}\\
\hspace{1em}intercept (shade, low light) & 2.92 [2.74, 3.08]\\
\hspace{1em}effect of sun growth treatment on $\log(l_\mathrm{gc})$ & 0.10 [0.09, 0.12]\\*
...
\midrule
\cellcolor{gray!10}{0.00} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{\cmark}\\
0.49 & \cmark & \cmark & \cmark & \\
\cellcolor{gray!10}{0.61} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{}\\
11.34 & \cmark & \cmark & \cmark & \cmark\\
\cellcolor{gray!10}{39.06} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{}\\
\addlinespace
41.37 & \cmark &  &  & \\
\cellcolor{gray!10}{43.46} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark}\\
45.92 & \cmark & \cmark &  & \cmark\\
\cellcolor{gray!10}{66.95} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{}\\
83.83 &  &  & \cmark & \cmark\\
\addlinespace
\cellcolor{gray!10}{85.30} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{\cmark}\\
86.39 &  &  & \cmark & \\
\cellcolor{gray!10}{96.02} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark}\\
99.92 &  &  &  & \\
\cellcolor{gray!10}{101.00} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{} & \cellcolor{gray!10}{\cmark}\\
\addlinespace
103.59 &  & \cmark &  & \\
\bottomrule
\end{tabular}
