...
described in (10). The idea to use gsw response curve to test hypotheses
about which traits influence stomatal kinetics came after the data had
been collected, but not yet analyzed. We started with an \emph{a priori}
hypothesis that guard cell size would influence \(\tau\) based on a
recent metaanalysis of stomatal responses to light (2) and we planned to
compare relationships within and among species, as described in this
paper. We made the decision to test for an association between fgmax and
\(\tau\) after exploratory analyses revealed a potential association.
Adding hypotheses after some results have been analyzed, also known as
``Hypothesizing After Results Known'' (HARKing), can lead to reports of
spurious associations that are not repeatable in later studies (cite).
...
phylogenetic heritability was estimated following (11); (12) as:
...
\subsubsection{Phylogenetic heritability
(somewhere)}\label{phylogenetic-heritability-somewhere}

Should among individual variance be included in calculation of
phylogenetic heritability? Consult de Villem? and Nagasaki? paper
...
\(tau\)), the preponderance of the variance was within or between
individuals after accounting for treatment effects
Figure~\ref{fig-variance}, Table~\ref{tbl-variance}. The exception to
this pattern was \gcl{}, for which the phylogenetic component was larger
Figure~\ref{fig-variance}. The phylogenetic variance component was
significantly higher than that of other response variables
Figure~\ref{fig-variance}. add table to show CIs in supplement?
...
\caption{\label{tbl-variance}Variance components and phylogenetic
heritability of stomatal kinetic traits.}
...
\midrule
\endfirsthead
\caption[]{Variance components and phylogenetic heritability of stomatal kinetic traits. \textit{(continued)}}\\
\toprule
component & \% variance & 95\% CI\\
\midrule
\endhead
...
Variation in stomatal kinetic parameters were associated with variation
in guard cell size and fgmax at different levels of organization. We
focus first on untreated leaves from plants grown and measured under
high light intensity treatments. The phylogenetic variance in guard cell
size among accessions was X \(\times\) greater than the variance among
individuals within the same accession WORKING HERE - GET THIS NUMBER
FROM MODEL.

Need to show (this is a growing, incomplete list): - plot of tau and
lambda versus t50 to show that the latter is more sensitive to tau -
guard cell size is less variable than stomatal density and fgmax -
partial correlation showing that gcl and tau are (only?) significant
phylogenetic covariates
