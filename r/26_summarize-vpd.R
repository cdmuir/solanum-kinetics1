# Reviewer comment R1.1: summarize curve-level VPD covariates (median VPD,
# final VPD, and rate of VPD change) and check how correlated they are with
# each other before deciding how/whether to include them in the model.
#
...
# rescue all of them.
#
# As a sensitivity/validation check, the same saturating model is also fit
# to the two raw humidity channels the VPD calculation is built from: H2O_s
# (sample cell, i.e. the air actually exchanged with the leaf) and H2O_r
# (reference cell, i.e. incoming air upstream of the leaf). We expect H2O_s
# to behave like VPD (both reflect leaf transpiration declining as stomata
# close), and H2O_r to be comparatively flat (it should mostly reflect the
# controlled/scrubbed incoming air, not the leaf response). This is a check,
# not a covariate source -- see bottom of script.
...
# the rate constant is poorly identified. Excluded here, alongside curves
# with r2_sat < 0.8, before use as a covariate/measurement-error term.
